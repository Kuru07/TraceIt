...
class PhoneClient (
    private val httpClient: HttpClient
        ...
        suspend fun getPhoneValidation(phone: String): Result<VoipResponse, NetworkError> {
            val response = try {
                httpClient.get(
                    urlString = "https://phonevalidation.abstractapi.com/v1"
                ){
                    parameter("api_key", ABSTRACT_API_KEY)
            ...
            return when(response.status.value){
                in 200..299 -> {
                    ...
                    val voipResponse : VoipResponse = response.body()
                    Result.Success(voipResponse)
                }
